using Plots, Random, StatsPlots, Bijectors, Colors
...
pathfinder_1_cell = read(f, Chains)
...
histogram(prior_samples, alpha = 0.8, label = "Prior", color="cornflowerblue")
histogram!(pathfinder_1_cell[:P₀], nbins=20, label = "Posterior", alpha = 0.6, color="pink")  
plot!(xlabel = "P₀", ylabel = "frequency")
savefig("prior_vs_posterior_pathfinder_p0.pdf")

plot()
prior_samples = rand(h_dist, 1000)
histogram(prior_samples, label = "Prior", color="cornflowerblue")
histogram!(pathfinder_1_cell[:h], nbins=40, label = "Posterior", alpha = 0.6, color="pink")
plot!(xlabel = "h", ylabel = "frequency")
savefig("prior_vs_posterior_pathfinder_h.pdf")

plot()
prior_samples = rand(tau_dist, 1000)
histogram(prior_samples, label = "Prior", color="cornflowerblue")
histogram!(pathfinder_1_cell[:τ], nbins=40, label = "Posterior", alpha = 0.6, color="pink")
plot!(xlabel = "τ", ylabel = "frequency")
savefig("prior_vs_posterior_pathfinder_tau.pdf")
