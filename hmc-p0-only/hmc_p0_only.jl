...
#chain
hmc_p0_2000 = sample(model, NUTS(1000, 0.8), MCMCThreads(), 2000, 4) # summary statistics

# just the generated values
A = Array(hmc_p0_2000, append_chains=false)
...
#traceplot
hmc_p0_traceplot = plot(hmc_p0_2000, seriestype = :traceplot, margin = 10mm)
...
hmc_p0_densityplot = density(hmc_p0_2000[:P₀], lw=2, label=chain_label, margin=10mm)
plot!(title="P₀", xlabel="Parameter values", ylabel = "Density")
vline!([3407.99], color=black, lw=3, line=:dashdot, label="Ground truth", legend=:topright, alpha=0.7)
